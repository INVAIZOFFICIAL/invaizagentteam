  ...
  try {
    ...
    do {
      const res = await notionClient.databases.query({
        ...
        filter: {
          and: [
            { property: '카테고리', select: { equals: '레퍼런스콘텐츠' } },
      ...
      for (const page of res.results) {
        ...
        const summaryArr = p.properties['한줄요약']?.rich_text ?? [];
        const summary = summaryArr.map((t: { plain_text?: string }) => t.plain_text ?? '').join('');
        ...
        const handleMatch = title.match(HANDLE_REGEX);
